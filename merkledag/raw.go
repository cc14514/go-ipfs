...
func NewRawNode(data []byte) *RawNode {
	...
	dlog.Println("merkledag/raw.go : NewRawNode : cid =",c.String(),", blk.data.size =",len(data))
	return &RawNode{blk}
}
...
func NewRawNodeWPrefix(data []byte, prefix cid.Prefix) (*RawNode, error) {
	...
	//dlog.Println("merkledag/raw.go : NewRawNodeWPrefix : cid =",c.String(),", blk.data.size =",len(data),err)
	if err != nil {
		return nil, err
	}
