...
import (
	...
	upb "github.com/cc14514/go-ipfs/unixfs/pb"
...
func (w *Writer) WriteNode(nd node.Node, fpath string) error {
	switch nd := nd.(type) {
	case *mdag.ProtoNode:
		dlog.Println("ProtoNode TTTT> data_size =",len(nd.Data()),nd.String())
		pb := new(upb.Data)
		if err := proto.Unmarshal(nd.Data(), pb); err != nil {
			return err
		}
		dlog.Println("ProtoNode TTTTTTTT> pb_data_size =",len(pb.GetData()),",pb_filesize =",pb.GetFilesize(),", type =",pb.GetType(),", fpath =",fpath)
		// debug >>>>
		/*
		...
		*/
		...
		case upb.Data_Raw:
			fallthrough
		case upb.Data_File:
			dlog.Println("ProtoNode TTTTTTTTTTTT> Data_File",pb.GetType())
	...
	case *mdag.RawNode:
		dlog.Println("RawNode TTTT> data_size =",len(nd.RawData()),nd.String())
		if err := writeFileHeader(w.TarW, fpath, uint64(len(nd.RawData()))); err != nil {
			return err
		}

		dlog.Println("RawNode TTTTTTTT> writeFileHeader :",fpath)
		if _, err := w.TarW.Write(nd.RawData()); err != nil {
			return err
		}
		w.TarW.Flush()
		dlog.Println("RawNode TTTTTTTTTTTT> write and flush :",len(nd.RawData()))
		return nil
	default:
