...
func NewPBFileReader(ctx context.Context, n *mdag.ProtoNode, pb *ftpb.Data, serv mdag.DAGService) *pbDagReader {
	...
	debuglogger.Println("NewPBFileReader >>> promises.len :=",len(promises))
	return &pbDagReader{
		node:     n,
		serv:     serv,
...
func (dr *pbDagReader) precalcNextBuf(ctx context.Context) error {
	debuglogger.Println("precalcNextBuf 1")
	dr.buf.Close() // Just to make sure
	if dr.linkPosition >= len(dr.promises) {
		return io.EOF
	}
	debuglogger.Println("precalcNextBuf 2")
	...
	dr.linkPosition++
	debuglogger.Println("precalcNextBuf 3")

	switch nxt := nxt.(type) {
	case *mdag.ProtoNode:
		debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : <<ProtoNode>> ")
		pb := new(ftpb.Data)
		...
		case ftpb.Data_Directory:
			debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : Data_Directory",pb.GetType())
			// A directory should not exist within a file
			return ft.ErrInvalidDirLocation
		case ftpb.Data_File:
			debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : Data_File",pb.GetType())
			dr.buf = NewPBFileReader(dr.ctx, nxt, pb, dr.serv)
			return nil
		case ftpb.Data_Raw:
			debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : Data_Raw",pb.GetType())
			dr.buf = NewBufDagReader(pb.GetData())
			return nil
		case ftpb.Data_Metadata:
			debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : Data_Metadata",pb.GetType())
			return errors.New("shouldnt have had metadata object inside file")
		case ftpb.Data_Symlink:
			debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : Data_Symlink",pb.GetType())
	...
	default:
		var err error
		debuglogger.Println("unixfs/io/pbdagreader.go : precalcNextBuf : <<NOT:ProtoNode>> ",nxt)
...
func (dr *pbDagReader) WriteTo(w io.Writer) (int64, error) {
	...
	for {
		...
		debuglogger.Println(">>w>> offset =",dr.offset, "pos =",dr.linkPosition,"total =",total)
		...
		// Otherwise, load up the next block
		debuglogger.Println("precalcNextBuf >>>>")
		err = dr.precalcNextBuf(dr.ctx)
		debuglogger.Println("precalcNextBuf <<<<",err)
		if err != nil {
			if err == io.EOF {
				return total, nil
			}
