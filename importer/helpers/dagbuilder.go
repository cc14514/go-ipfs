...
func (db *DagBuilderHelper) GetNextDataNode() (*UnixfsNode, error) {
	...
	//dlog.Println("importer/helpers/dagbuilder.go : GetNextDataNode : db.rawLeaves =",db.rawLeaves)
	if db.rawLeaves {
		//dlog.Println("importer/helpers/dagbuilder.go : GetNextDataNode : db.prefix =",db.prefix)
		if db.prefix == nil {
			return &UnixfsNode{
