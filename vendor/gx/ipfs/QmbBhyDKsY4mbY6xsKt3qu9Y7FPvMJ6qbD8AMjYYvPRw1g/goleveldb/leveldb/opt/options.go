...
var (
	...
	DefaultBlockRestartInterval          = 16
	DefaultBlockSize                     = 3 * KiB // 这个影响了 buffer_pool.go 使用的内存数量
	DefaultCompactionExpandLimitFactor   = 25
	DefaultCompactionGPOverlapsFactor    = 10
	DefaultCompactionL0Trigger           = 4
	DefaultCompactionSourceLimitFactor   = 1
	DefaultCompactionTableSize           = 2 * MiB
	DefaultCompactionTableSizeMultiplier = 1.0
	DefaultCompactionTotalSize           = 5 * MiB
	DefaultCompactionTotalSizeMultiplier = 5.0
	DefaultCompressionType               = SnappyCompression
	DefaultIteratorSamplingRate          = 512 * KiB
	DefaultOpenFilesCacher               = LRUCacher
	DefaultOpenFilesCacheCapacity        = 250
	DefaultWriteBuffer                   = 1 * MiB
	DefaultWriteL0PauseTrigger           = 12
	DefaultWriteL0SlowdownTrigger        = 8
)
