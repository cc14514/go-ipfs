...
import (
	...
	"github.com/cc14514/go-ipfs/core/commands"
	"github.com/cc14514/go-ipfs/core/corehttp"
	"github.com/cc14514/go-ipfs/core/corerepo"
	nodeMount "github.com/cc14514/go-ipfs/fuse/node"
	"github.com/cc14514/go-ipfs/repo/fsrepo"
	migrate "github.com/cc14514/go-ipfs/repo/fsrepo/migrations"
...
`,
	...
	Options: []cmds.Option{
		// add by liangc
		cmds.StringOption(homepath, "for set env of IPFS_PATH"),
...
func daemonFunc(req cmds.Request, res cmds.Response) {
	...
	// add by liangc : 启动一个服务, 负责传递帮助消息
	helpservice.NewHelpService(node)
	...
	//add by liangc : natOfAPI 在 serveHTTPApi 中初始化
	defer func(){
		debuglogger.Println(">>>> nat_of_api_close. <<<<")
		if natOfAPI != nil {
			natOfAPI.Close()
		}
	}()
...
func serveHTTPApi(req cmds.Request) (error, <-chan error) {
	...
	//======================================================
	// add by liangc : api port nat mapping >>>>>>>
	//======================================================
	/*
	discoverdone := make(chan struct{})
	go func() {
		defer close(discoverdone)
		debuglogger.Println("nat begin")
		natOfAPI = inat.DiscoverNAT()
		debuglogger.Println("nat end")
	}()
	...
	select {
	case <-discoverdone:
		if natOfAPI == nil { // no nat, or failed to get it.
			debuglogger.Println("⚠️ corehttp nat not found.")
		} else {
			natOfAPI.Process()
			debuglogger.Println("nat successed.")
			mapping, err := natOfAPI.NewMapping(apiMaddr)
			n := mapping.NAT()
			//TODO 这个映射端口的回收，要放到上层去，否则映射生命周期太短
			defer func() {
				debuglogger.Println(" TODO ********* TODO 👋 ********")
				debuglogger.Println(" TODO ********* TODO 👋 ********")
				//n.Close()
			}()
			debuglogger.Println(n)
			for i,a := range natOfAPI.ExternalAddrs() {
				debuglogger.Println("🔗  ea=", a, " : ",i,err)
			}
		}
	}
	*/
	//======================================================
	// add by liangc : apid port nat mapping <<<<<<<<
	//======================================================
