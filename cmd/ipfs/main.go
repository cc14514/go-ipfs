...
import (
	...
	cmdsCli "github.com/cc14514/go-ipfs/commands/cli"
	cmdsHttp "github.com/cc14514/go-ipfs/commands/http"
	core "github.com/cc14514/go-ipfs/core"
	coreCmds "github.com/cc14514/go-ipfs/core/commands"
	"github.com/cc14514/go-ipfs/plugin/loader"
	repo "github.com/cc14514/go-ipfs/repo"
	config "github.com/cc14514/go-ipfs/repo/config"
	fsrepo "github.com/cc14514/go-ipfs/repo/fsrepo"
...
var EnableProfile = false // 只有 daemon 时才可以启动这个功能
const (
...
func main() {
	// TODO 10% 频繁的 GC 会不会降低内存占用率？
	debug.SetGCPercent(10)
	// 限制线程数会降低内存使用率么？
	// 事实证明不行 fatal error: thread exhaustion
	//debug.SetMaxThreads(20)
	/*
	//通过控制内核数，来降低执行效率
	*/
	if cn := runtime.NumCPU();cn > 1 {
		//fmt.Println("numcpu =",cn)
		runtime.GOMAXPROCS(1)
		//runtime.GOMAXPROCS(int(cn/2))
		//fmt.Println("maxprocs =",runtime.GOMAXPROCS(0))
	}

	for _, arg := range (os.Args) {
		//fmt.Println(i, arg)
		if p := strings.Index(arg, "--homepath"); p == 0 {
			arr := strings.Split(arg,"=")
			path := arr[1]
			fmt.Println("==== HOME ====>",path)
			os.Setenv("IPFS_PATH",path)
		}
		if strings.Contains(arg,"daemon") {
			fmt.Println("Enable profile == true")
			EnableProfile = true
		}
	}
	fmt.Println(">>>>",DevFlag," <<<<")
...
func writeHeapProfileToFile() error {
	...
	if err != nil {
		fmt.Println("❌ writeHeapProfileToFile_err =",err)
		return err
	}
	defer mprof.Close() // _after_ writing the heap profile
	fmt.Println("✅ writeHeapProfileToFile_successed")
...
func profileIfEnabled() (func(), error) {
	...
	// works as intended.
	if os.Getenv(EnvEnableProfiling) != "" && EnableProfile {
		// add by liangc >>>>
		go func() {
			fmt.Println(">> http pprof start on : http://127.0.0.1:6060/debug/pprof/")
